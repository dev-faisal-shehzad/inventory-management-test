class Product < ApplicationRecord
  ...
  #   sql = <<~SQL
  #     SELECT GREATEST(
  #       SUM(COALESCE(order_line_items.quantity, 0) - COALESCE(SELECT on_shelf FROM products WHERE products.id = #{id}), 0)))
  ...
  #     ActiveRecord::Base.send(:sanitize_sql_array, [sql, { id: id }])
  #   )
